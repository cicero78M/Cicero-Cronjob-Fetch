...
| `cronDirRequestFetchSosmed.js` | `0,30 6-21 * * *<br>0 22 * * *` | Fetch Instagram/TikTok posts for all active clients (direktorat + org) with Instagram/TikTok enabled, refresh engagement metrics, lalu menulis event notifikasi ke `wa_notification_outbox` (status `pending`) dengan `idempotency_key` untuk deduplikasi. State notifikasi per client (`last_ig_count`, `last_tiktok_count`, `last_notified_at`, `last_notified_slot`) disimpan di tabel `wa_notification_scheduler_state`; trigger hourly berbasis slot global Jakarta (`currentSlotKey`) dan slot hanya diupdate saat enqueue sukses. |
