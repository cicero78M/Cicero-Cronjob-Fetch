...
| `cronDirRequestFetchSosmed.js` | `0,30 6-21 * * *<br>58 20-21 * * *` | Menjalankan refresh engagement untuk semua client aktif pada setiap run, sementara fetch post dibatasi runtime per segmen client: **Segmen A** (`org`/`ditbinmas`) sampai slot final **20:58 WIB**, **Segmen B** (`direktorat` selain `ditbinmas`) sampai slot final **21:58 WIB**. Di luar slot segmen, proses tetap jalan namun post fetch diskip (`skipPostFetch=true`). Job ini menulis event notifikasi ke `wa_notification_outbox` (status `pending`) dengan deduplikasi `idempotency_key`; notifikasi hanya dipicu oleh perubahan aktual (`hasNotableChanges`) dan state scheduler difokuskan pada baseline count per client (`last_ig_count`, `last_tiktok_count`). |
