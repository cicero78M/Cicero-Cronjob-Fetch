...
| `cronDirRequestFetchSosmed.js` | `0,30 6-21 * * *<br>58 20-21 * * *` | Menjalankan jadwal gabungan **slot `:00/:30`** ditambah **slot final `:58`** dengan segmentasi runtime per client: **Segmen A** (`org`/`ditbinmas`) sampai **20:58 WIB**, **Segmen B** (`direktorat` selain `ditbinmas`) sampai **21:58 WIB**. Setiap run tetap refresh engagement, sedangkan post fetch diskip di luar slot valid segmen (`skipPostFetch=true`). Event notifikasi ditulis ke `wa_notification_outbox` (status `pending`, deduplikasi `idempotency_key`) dan **daftar tugas hanya dikirim saat terdeteksi perubahan data post/link** (`hasNotableChanges`). State scheduler dipakai untuk baseline count per client (`last_ig_count`, `last_tiktok_count`) tanpa mekanisme blast terjadwal per jam. |
