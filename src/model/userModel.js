...
export async function updateUserFieldWithSessionVersion({
  userId,
  field,
  value,
  ...
  if (["nama", "title", "divisi", "jabatan", "desa"].includes(field) && typeof committedValue === "string") {
    committedValue = committedValue.toUpperCase();
  }
  if (field === "whatsapp") {
    committedValue = normalizeWhatsappField(committedValue);
  }

  if (field === "client_id") {
    committedValue = typeof committedValue === "string" ? committedValue.toUpperCase() : committedValue;
    const { rows } = await query(
  ...
  const { rows } = await query(
    `UPDATE "user"
     ...
     RETURNING *`,
    [committedValue, uid, Number(expectedSessionVersion) || 0],
  );
  ...
  return {
    ...
    user: rows[0],
    sessionVersion: Number(rows[0].session_version || 0),
  };
}
